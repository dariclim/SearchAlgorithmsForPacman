...
class CornersProblem(search.SearchProblem):
    ...
    def expand(self, state):
        ...
        for action in self.getActions(state):
            ...
            child_cost = self.getActionCost(state, action, child)
            children.append((child, action, child_cost))
    ...
    def getNextState(self, state, action):
        ...
        new_loc = (nextx, nexty)
        if new_loc in self.corners and new_loc not in visited:
...
def cornersHeuristic(state, problem):
    ...
    for c in corners:
        if c not in visited:
            unvisited[c] = abs(x - c[0]) + abs(y - c[1])

    if unvisited == {}:
        return 0
    else:
        return unvisited[min(unvisited)]
...
def foodHeuristic(state, problem):
    ...
    for i in foodList:
        curr = mazeDistance(position, i, problem.startingGameState)
        all_dist.append(curr)
    if not all_dist:
        return 0
    else:
        return max(all_dist)
